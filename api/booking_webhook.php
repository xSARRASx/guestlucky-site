...
function gmail_send(string $accessToken, string $from, string $to, string $subject, string $htmlBody, string $replyTo = '', string $cc = ''): bool {
      ...
      <table width="580" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 8px 40px rgba(26,37,71,.12);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1a2547 0%,#2d3f7a 100%);padding:28px 36px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <img src="https://www.locationcourteduree.fr/wp-content/uploads/2026/05/logo-guestlucky.png"
        ...
        <tr>
          <td style="padding:36px 36px 0;">
            <div style="width:56px;height:56px;background:linear-gradient(135deg,#ede9fe,#ddd6fe);border-radius:16px;display:inline-flex;align-items:center;justify-content:center;font-size:26px;margin-bottom:20px;">
              {$typeIcon}
            </div>
...
// Étape 6 — Récupérer les credentials Google de l'agent
...
if (!$agentRecord) {
    ...
    if ($refreshToken) {
        // Étape 7 — Rafraîchir l'access token Google
        $accessToken = google_refresh_access_token($refreshToken);

        if ($accessToken) {
            // Étape 8 — Créer l'événement Google Calendar
            $summary     = "{$booking['type_icon']} {$booking['type_label']} — {$booking['client_name']}";
            $description = implode("\n", [
                ...
                "",
                "Réservé via lumnistech.fr — Airtable ID : {$airtableId}",
            ]);
            ...
            $meetLink    = gcal_create_event(
                ...
                $booking['timezone'] ?: 'Europe/Paris'  // fuseau du client
            );
            ...
            // Étape 9 — Envoyer l'email de confirmation Gmail
