...
$minNoticeHours  = (int)($typeFields['MinNoticeHours'] ?? 0);
...
if ($cachedBusy) {
    ...
    if ($tokenRes['code'] !== 200 || empty($tokenRes['body']['access_token'])) {
        $errorCode = $tokenRes['body']['error'] ?? 'unknown';
        $isExpired = in_array($errorCode, ['invalid_grant', 'token_has_been_expired_or_revoked']);
    
        log_line('error', $isExpired ? 'TOKEN_EXPIRED' : 'Token refresh échoué', [
            'code'  => $tokenRes['code'],
            'error' => $errorCode,
        ]);
    
        if ($isExpired) {
            $alertTo = $ENV['ALERT_EMAIL'] ?? '';
            if ($alertTo) {
                $subject = '🚨 GuestLucky — Token Google expiré';
                $body    = "Le refresh token Google est expiré ou révoqué.\n"
                         . "Les disponibilités sont en mode théorique.\n\n"
                         . "Action requise :\n"
                ...
                mail($alertTo, $subject, $body,
                    "From: noreply@guestlucky.com\r\nContent-Type: text/plain; charset=UTF-8"
                );
                log_line('info', 'Alerte token expiration envoyée', ['to' => $alertTo]);
            }
        }
...
$tz        = new DateTimeZone('Europe/Paris');
$now       = new DateTime('now', $tz);
$noticeMinutes = $minNoticeHours > 0
    ? $minNoticeHours * 60
    : BUFFER_MINUTES;
$scanStart = (clone $now)->modify("+{$noticeMinutes} minutes");

log_line('debug', 'Fenêtre de blocage appliquée', [
    'minNoticeHours' => $minNoticeHours,
    'noticeMinutes'  => $noticeMinutes,
    'scanStart'      => $scanStart->format('Y-m-d H:i'),
    'source'         => $minNoticeHours > 0 ? 'airtable' : 'buffer_minutes_default',
]);
$today     = new DateTime('today', $tz);
